...
O Actions `35240877511` recriou o banco duas vezes até `045` e aprovou as 74 provas pgTAP, além de 115 testes unitários, build e 26 E2E. A fonte `aca0affd44728b8c371d688f4021a46a0e446ac5` foi publicada como versão 46 no Sites, preservando a integração PostHog já adicionada diretamente ao repositório de hospedagem. O `db push` remoto não alterou a homologação: as oito conexões temporárias ao pooler expiraram. Aplicar `045` pelo SQL Editor e registrar `202609160045` em `supabase_migrations.schema_migrations` antes de homologar o upload e o carrossel. Também revisar institucionalmente privacidade, consentimento e retenção do PostHog antes da produção real.
