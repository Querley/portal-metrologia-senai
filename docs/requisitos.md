...
5. Receber respostas configuráveis e até cinco arquivos por solicitação: PDF/imagem até 10 MB; CAD até 50 MB; armazenamento privado, visualização interna de PDF/imagem e download explícito.
...
- A barra da etapa permite selecionar o progresso por clique ou arraste; a alteração só é persistida após “Salvar progresso”, preservando sequência, autorização e estados coerentes.
...
- Imagens e vídeos da página inicial podem variar por idioma. URLs aceitas são caminhos públicos do portal ou HTTPS; os quatro setores e cada novidade admitem carrossel próprio com proporção estável, legenda e descrição acessível obrigatórias; fichas descritivas de equipamento não pertencem ao escopo editável do CMS.
