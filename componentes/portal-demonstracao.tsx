...
const solicitacoes = [
  {
    id: 'SOL-0284',
    empresa: 'Indústria Horizonte',
    servico: 'Medição tridimensional',
    data: 'Hoje, 09:42',
    estado: 'Nova',
  },
  {
    ...
    servico: 'Digitalização óptica',
    data: 'Ontem, 16:18',
    estado: 'Em análise',
  },
  {
    id: 'SOL-0282',
    empresa: 'Fábrica Vetor',
    servico: 'Raios X industrial',
    data: '20 ago, 11:05',
    estado: 'Orçada',
  },
];
...
export function PortalDemonstracao({ nomeUsuario = 'Usuário Demo', perfilUsuario = 'Administrador', perfilInterno, clienteSupabase, aoSair, aoAbrirPerfil, autenticado = false }: PropriedadesPortal) {
      ...
      calcularProposta([
        {
          ...
          descricao: 'Inspeção dimensional de lote',
          quantidade: '20',
          usos: [
            {
              maquinaId: 'duramax',
              horas: String(horas),
              custoHora: equipamentosDemonstracao[0].custoHora,
            },
          ],
          custosExtras: '280',
          percentualLucro: String(lucro),
        },
      ])[0],
    [horas, lucro],
  );
      ...
      recomendarHoras(
        {
          ...
          servicoId: 'medicao-tridimensional',
          quantidade: 20,
          caracteristicas: ['aco', 'geometria-complexa'],
          recursos: ['duramax'],
        },
        casosDemonstracao,
      ),
  ...
  const previaIa = sanitizarParaIa({
    tipoServico: 'Medição tridimensional',
    calculo: { horas, custoTecnico: 'removido antes do envio' },
  ...
  return (
    <div className="aplicacao" data-hidratado={hidratado ? 'sim' : 'nao'}>
      <aside className="barra-lateral">
        <a className="marca marca-interna" href="/" aria-label="Voltar à página pública">
          <MarcaOficial />
          <span className="marca-interna-legenda">Gestão de serviços e conhecimento</span>
        </a>
        <div className="faixa-demo">
          <span>{autenticado ? 'HOMOLOGAÇÃO' : 'DEMONSTRAÇÃO LOCAL'}</span>
          <small>Dados sintéticos isolados</small>
        </div>
        ...
        <div className="atalhos">
          <button type="button" onClick={aoAbrirPerfil} disabled={!aoAbrirPerfil}>
            <Settings size={17} />
            Meu perfil
          </button>
      ...
      <main className="conteudo-interno">
        <header className="cabecalho-interno">
          <div>
            ...
            <h1>{menu.find((itemMenu) => itemMenu.id === secao)?.rotulo}</h1>
          </div>
          <div className="controles-cabecalho-interno">
            <div className="acoes-conta-responsivas" aria-label="Conta e sessão">
              <button type="button" onClick={aoAbrirPerfil} disabled={!aoAbrirPerfil} title="Meu perfil">
                <Settings size={18} />
                <span>Meu perfil</span>
              </button>
              <button type="button" onClick={() => (aoSair ? void aoSair() : window.location.assign('/'))} title="Sair">
                <LogOut size={18} />
                <span>Sair</span>
              </button>
            </div>
              ...
              <div className="acoes-internas">
                ...
                <button className="botao-interno" type="button" onClick={() => setSecao(perfilInterno && podeCriarRascunhoOrcamento(perfilInterno) ? 'solicitacoes' : 'orcamentos')}>
                  {perfilInterno && !podeCriarRascunhoOrcamento(perfilInterno) ? (
                    ...
                    <>
                      <Plus size={17} /> Nova pré-proposta
                    </>
                  )}
                </button>
              </div>
      ...
      {secao !== 'custos' && !(secao === 'orcamentos' && orcamentosPersistentesDisponiveis) && !(secao === 'mensagens' && mensagensPersistentesDisponiveis) && (
        <button className="assistente-atalho" type="button" onClick={() => setAssistenteAberto(!assistenteAberto)} aria-expanded={assistenteAberto}>
          <Sparkles size={19} /> Assistente interno
        </button>
      )}
      {assistenteAberto && secao !== 'custos' && !(secao === 'orcamentos' && orcamentosPersistentesDisponiveis) && !(secao === 'mensagens' && mensagensPersistentesDisponiveis) && (
        <div
          className="fundo-assistente"
          role="presentation"
          onMouseDown={(evento) => {
            if (evento.target === evento.currentTarget) setAssistenteAberto(false);
          }}
        >
          <aside className="assistente" aria-label="Assistente interno">
            <header>
              <span>
                <Bot size={19} /> Assistente interno
              </span>
              <button type="button" onClick={() => setAssistenteAberto(false)} aria-label="Fechar">
                ×
              </button>
            </header>
            <div className="mensagem-ia">
              <small>PRÉVIA SANITIZADA</small>
              <p>{JSON.stringify(previaIa.conteudo)}</p>
              <span>
                <ShieldCheck size={14} /> Campos sensíveis removidos antes do envio
              </span>
            </div>
            <div className="resposta-ia">
              <p>O preço exibido foi calculado de forma determinística: custo da DuraMax pelas horas informadas, mais extras e o lucro definido. Eu apenas explico o resultado; não altero o cálculo.</p>
              <small>Fonte: orçamento atual · cálculo local</small>
            </div>
            <form onSubmit={(evento) => evento.preventDefault()}>
              <label htmlFor="mensagem-assistente">Pergunte sobre esta tela</label>
              <div>
                <input id="mensagem-assistente" placeholder="Como este valor foi calculado?" />
...
function VisaoGeral({ setSecao }: { setSecao: (secao: string) => void }) {
  const cards = [
    ...
    },
    {
      rotulo: 'Em execução',
      valor: '8',
      detalhe: '7 dentro do prazo',
      icone: Activity,
      cor: 'ciano',
    },
    {
      rotulo: 'Propostas aguardando',
      valor: '5',
      detalhe: 'R$ 48,7 mil',
      icone: FileCheck2,
      cor: 'amarelo',
    ...
    {
      rotulo: 'Assertividade mensal',
      valor: '87%',
      detalhe: '+6 p.p. vs. julho',
      icone: Gauge,
      cor: 'verde',
    },
  ];
  return (
    <div className="painel">
      <section className="cards-kpi" aria-label="Indicadores principais">
        {cards.map(({ rotulo, valor, detalhe, icone: Icone, cor }) => (
          <article key={rotulo}>
            <span className={`icone-kpi ${cor}`}>
              <Icone size={20} />
            </span>
            <small>{rotulo}</small>
            <strong>{valor}</strong>
        ...
        <section className="bloco">
          <header>
            <div>
              ...
              <p>Priorize as entradas que aguardam triagem.</p>
            </div>
            <button type="button" onClick={() => setSecao('solicitacoes')}>
              Ver todas <ChevronRight size={15} />
            </button>
          </header>
          <TabelaSolicitacoes compacta />
        </section>
        <section className="bloco fluxo">
          <header>
            <div>
              <h2>Ciclo de valor</h2>
              <p>Distribuição dos serviços ativos.</p>
            </div>
          </header>
          <div className="anel" aria-label="72% do ciclo concluído">
            <span>
              <strong>72%</strong>
              <small>no fluxo</small>
            </span>
          </div>
          <ul>
            <li>
      ...
      <section className="alerta-conhecimento">
        <BookOpenCheck size={23} />
        <div>
          <strong>4 lições aguardam validação</strong>
          <p>Formalizá-las pode melhorar a confiança das próximas recomendações.</p>
        </div>
        <button type="button" onClick={() => setSecao('conhecimento')}>
          Revisar lições
        </button>
      </section>
    </div>
...
function TabelaSolicitacoes({ compacta = false }: { compacta?: boolean }) {
  return (
    <div className={`tabela-wrap ${compacta ? 'compacta' : 'pagina-tabela'}`}>
      <table>
        <thead>
          <tr>
            <th>Solicitação</th>
        ...
        <tbody>
          {solicitacoes.map((solicitacao) => (
            <tr key={solicitacao.id}>
              ...
              <td>
                <span className={`estado estado-${solicitacao.estado.toLowerCase().replace(' ', '-')}`}>{solicitacao.estado}</span>
              </td>
              <td>
                <button type="button" aria-label={`Abrir ${solicitacao.id}`}>
      ...
      {!compacta && (
        <div className="estado-vazio">
          <ShieldCheck size={18} />
          <span>
            Todos estes registros têm origem <b>demonstracao</b>.
          </span>
        </div>
      )}
    </div>
...
function Calculadora({ horas, setHoras, lucro, setLucro, item, recomendacao }: { horas: number; setHoras: (valor: number) => void; lucro: number; setLucro: (valor: number) => void; item: ReturnType<typeof calcularProposta>[number]; recomendacao: ReturnType<typeof recomendarHoras> }) {
  return (
    <div className="painel orcamento">
      <section className="bloco editor-orcamento">
        <header>
          <div>
            <p className="passo">ORÇAMENTO DEMONSTRATIVO · RASCUNHO</p>
            <h2>Inspeção dimensional de lote</h2>
          </div>
          <span className="estado estado-rascunho">Rascunho</span>
        </header>
        <div className="campos">
          <label>
            Equipamento
            <select defaultValue="duramax">
              {equipamentosDemonstracao.map((equipamento) => (
          ...
          <label>
            Horas estimadas
            <input type="number" min="0" step="0.5" value={horas} onChange={(evento) => setHoras(Number(evento.target.value))} />
          </label>
          <label>
            Custos extras (BRL)
            <input type="number" min="0" defaultValue="280" />
          </label>
          <label>
            Lucro (%)
            <input type="number" min="0" value={lucro} onChange={(evento) => setLucro(Number(evento.target.value))} />
          </label>
        </div>
        <div className="recomendacao-inline">
          <Sparkles size={20} />
          <div>
            <strong>Assistente estatístico: {recomendacao.horasSugeridas?.toDecimalPlaces(1).toString()} h sugeridas</strong>
            <p>
              {recomendacao.quantidadeCasos} casos comparáveis · confiança {recomendacao.confianca} · faixa {recomendacao.q1?.toDecimalPlaces(1).toString()}–{recomendacao.q3?.toDecimalPlaces(1).toString()} h
            </p>
          </div>
          <button type="button" onClick={() => setHoras(recomendacao.horasSugeridas?.toDecimalPlaces(1).toNumber() ?? horas)}>
            Aplicar
          </button>
        </div>
      </section>
      <aside className="bloco resumo-orcamento">
        <p>RESUMO DO ITEM</p>
        <dl>
          <div>
            <dt>Custo das máquinas</dt>
            <dd>{formatarDinheiro(item.custo.minus(280))}</dd>
          </div>
          <div>
            <dt>Custos extras</dt>
            <dd>R$ 280,00</dd>
          ...
          <div>
            <dt>Custo total</dt>
            <dd>{formatarDinheiro(item.custo)}</dd>
          </div>
          <div>
            <dt>Lucro ({lucro}%)</dt>
            <dd>{formatarDinheiro(item.precoAntesAjuste.minus(item.custo))}</dd>
          </div>
          <div className="total">
            <dt>Preço antes do ajuste</dt>
            <dd>{formatarDinheiro(item.precoFinal)}</dd>
          </div>
        </dl>
        <small>
          <ShieldCheck size={13} /> Valores congelados somente após publicação
        </small>
        <button className="botao-interno" type="button">
          <FileCheck2 size={17} /> Enviar para validação
        </button>
      </aside>
    </div>
  );
...
function ExecucaoDemonstrativa() {
  return (
    <div className="painel">
      <section className="bloco estado-vazio-execucao">
        <Activity size={28} />
        <h3>Execução persistente indisponível</h3>
        <p>Entre na homologação autenticada para acompanhar e atualizar as etapas demonstrativas.</p>
      </section>
    </div>
  );
}

function Conhecimento({ recomendacao }: { recomendacao: ReturnType<typeof recomendarHoras> }) {
  return (
    <div className="painel">
      <section className="cards-kpi conhecimento-kpi">
        <article>
          ...
          <strong>23</strong>
          <p>4 aguardam validação</p>
        </article>
        <article>
          <span className="icone-kpi verde">
            <Gauge />
          </span>
          <small>Confiança atual</small>
          <strong>{recomendacao.confianca}</strong>
          <p>{recomendacao.quantidadeCasos} casos elegíveis</p>
        </article>
        <article>
          <span className="icone-kpi ciano">
            <Activity />
          </span>
          <small>Fator de correção</small>
          <strong>{recomendacao.fatorCorrecao?.toDecimalPlaces(2).toString()}×</strong>
          <p>mediana realizado ÷ estimado</p>
        </article>
      </section>
      <section className="bloco tabela-licoes">
        <header>
          <div>
            <h2>Conhecimento validado</h2>
            <p>Apenas estas lições influenciam recomendações.</p>
          </div>
          <button type="button">
            <Plus size={15} /> Nova lição
          </button>
        </header>
        {['Revisar estratégia de fixação antes da programação', 'Validar qualidade do CAD no recebimento', 'Reservar tempo para estabilização térmica'].map((titulo, indice) => (
          <article key={titulo}>
            <span className="numero-licao">L{23 - indice}</span>
            <div>
              <strong>{titulo}</strong>
              <p>Medição tridimensional · Revisão {indice + 1} · 18–20 ago</p>
            </div>
            <span className="estado estado-formalizada">Formalizada</span>
            <button type="button" aria-label={`Abrir lição ${titulo}`}>
              <ChevronRight size={17} />
            </button>
          </article>
        ))}
...
function Mensagens() {
  return (
    <div className="painel">
      <section className="bloco mensagens">
        <aside>
          <h2>Conversas</h2>
          {solicitacoes.slice(0, 2).map((item, indice) => (
            <button className={indice === 0 ? 'ativo' : ''} type="button" key={item.id}>
              <span>{item.empresa.slice(0, 2).toUpperCase()}</span>
              <div>
                <strong>{item.empresa}</strong>
                <small>
                  {item.id} · {indice ? 'Enviei os arquivos...' : 'Podemos confirmar...'}
                </small>
              </div>
              {indice === 0 && <b>2</b>}
            </button>
        ...
        <div className="conversa">
          <header>
            <div>
              <strong>Indústria Horizonte</strong>
              <small>SOL-0284 · Canal persistente</small>
            </div>
            <span className="estado estado-em-analise">Em análise</span>
          </header>
          <div className="baloes">
            <p className="recebida">
              Olá! Podemos confirmar a tolerância indicada no desenho?
              <small>09:58</small>
            </p>
            <p className="enviada">
              Sim, a tolerância crítica é de 0,01 mm. O arquivo da solicitação é a versão atual.<small>10:04</small>
            </p>
            <p className="recebida">
              Perfeito. Vamos considerar essa informação na proposta.
              <small>10:07</small>
            </p>
          </div>
          <form onSubmit={(evento) => evento.preventDefault()}>
            <input aria-label="Mensagem" placeholder="Escreva uma mensagem..." />
