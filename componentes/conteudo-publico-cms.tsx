...
type ItemAcontecimento = { tipo: 'recente' | 'agora' | 'proximo'; data: string; titulo: string; resumo: string; midias?: MidiaCms[] };
type SetorCms = { slug: string; midias: MidiaCms[] };
type CorpoCms = { texto?: string; midia_url?: string; midia_tipo?: 'imagem' | 'video'; midia_alt?: string; midias?: MidiaCms[]; itens?: ItemAcontecimento[]; setores?: SetorCms[] };
...
export function ConteudoPublicoCms({ cliente, perfil }: { cliente: SupabaseClient; perfil: Extract<PerfilInterno, 'validador' | 'administrador'> }) {
  ...
  const [setores, setSetores] = useState<SetorCms[]>(setoresIniciais);
  ...
  const ehSetores = chave === 'inicio.setores';
  const secoesInicio = useMemo(() => {
    const ordem = ['inicio.hero', 'inicio.diferencial', 'inicio.estrutura', 'inicio.setores', 'inicio.acontecimentos', 'inicio.chamada'];
  ...
  function carregarCampos(item: ConteudoCms, codigo: IdiomaPublico) {
    ...
    setItens(corpo.itens?.length ? corpo.itens : itensIniciais); setSetores(corpo.setores?.length ? corpo.setores : setoresIniciais); setJustificativa('');
  }
  ...
  function alterarItem(indice: number, campo: 'tipo' | 'data' | 'titulo' | 'resumo', valor: string) {
    setItens((atuais) => atuais.map((item, posicao) => posicao === indice ? { ...item, [campo]: valor } : item));
  }

  async function enviarMidia(arquivo?: File, destino: 'principal' | 'estrutura' | `setor:${string}` | `acontecimento:${number}` = 'principal') {
    ...
    if (error) setErro(error.message || 'Não foi possível enviar a mídia.');
      ...
      if (destino === 'estrutura') setMidias((atuais) => [...atuais, novaMidia]);
      else if (destino.startsWith('setor:')) setSetores((atuais) => atuais.map((setor) => setor.slug === destino.slice(6) ? { ...setor, midias: [...setor.midias, novaMidia] } : setor));
      else if (destino.startsWith('acontecimento:')) { const indice = Number(destino.slice(14)); setItens((atuais) => atuais.map((item, posicao) => posicao === indice ? { ...item, midias: [...(item.midias ?? []), novaMidia] } : item)); }
      else { setMidiaUrl(data.publicUrl); setMidiaTipo(novaMidia.tipo); }
      setMensagem(destino === 'principal' ? 'Mídia substituída. Salve o rascunho para vinculá-la a esta versão.' : 'Mídia adicionada ao carrossel. Ajuste a legenda e salve o rascunho.');
    }
  ...
  async function salvar(evento: React.FormEvent<HTMLFormElement>) {
    ...
    if (ehAcontecimentos && itens.some((item) => (item.midias ?? []).some((midia) => !midia.legenda.trim() || !midia.alt.trim()))) {
      setErro('Preencha a legenda e a descrição acessível de todas as mídias dos acontecimentos.'); return;
    }
    if (ehSetores && setores.some((setor) => setor.midias.length === 0 || setor.midias.some((midia) => !midia.legenda.trim() || !midia.alt.trim()))) {
      setErro('Cada setor precisa ter ao menos uma mídia com legenda e descrição acessível.'); return;
    }
    ...
    if (ehSetores) corpo.setores = setores;
    ...
    <section className="grade-cms">
      ...
      <form className="bloco editor-cms" onSubmit={salvar}>
        ...
        {selecionadoId ? <div className="campos-cms">
          ...
          {!ehSetores && <fieldset className="midia-editor-cms"><legend><ImageIcon size={16} /> Mídia desta versão</legend><label className="soltar-midia-cms" onDragOver={(evento) => evento.preventDefault()} onDrop={(evento) => { evento.preventDefault(); void enviarMidia(evento.dataTransfer.files[0]); }}><UploadCloud size={26} /><strong>{enviandoMidia ? 'Enviando arquivo…' : 'Arraste uma imagem ou vídeo para substituir a mídia atual'}</strong><span>ou clique para escolher · imagens até 20 MB · vídeos até 80 MB</span><input type="file" accept="image/*,video/mp4,video/webm" disabled={enviandoMidia} onChange={(evento) => void enviarMidia(evento.target.files?.[0])} /></label><label>Descrição acessível<input maxLength={240} value={midiaAlt} onChange={(evento) => setMidiaAlt(evento.target.value)} /></label>{urlMidiaSegura(midiaUrl) && <div className="previa-midia-cms">{midiaTipo === 'video' ? <video controls src={urlMidiaSegura(midiaUrl)} /> : <Image src={urlMidiaSegura(midiaUrl)} alt={midiaAlt || titulo} width={720} height={400} unoptimized />}</div>}{chave === 'inicio.estrutura' && <><label className="soltar-midia-cms adicionar-carrossel-cms" onDragOver={(evento) => evento.preventDefault()} onDrop={(evento) => { evento.preventDefault(); void enviarMidia(evento.dataTransfer.files[0], 'estrutura'); }}><Plus size={22} /><strong>Adicionar ao carrossel</strong><span>Arraste outro arquivo ou clique para escolher</span><input type="file" accept="image/*,video/mp4,video/webm" disabled={enviandoMidia} onChange={(evento) => void enviarMidia(evento.target.files?.[0], 'estrutura')} /></label>{midias.length > 0 && <div className="midias-carrossel-cms">{midias.map((item, indice) => <article key={`${item.src}-${indice}`}><span>{item.tipo === 'video' ? 'Vídeo' : 'Imagem'} {indice + 1}</span><button type="button" onClick={() => setMidias((atuais) => atuais.filter((_, posicao) => posicao !== indice))}><Trash2 size={14} /> Remover</button></article>)}</div>}</>}</fieldset>}
          {ehSetores && <fieldset className="editor-carrosseis-cms"><legend>Carrosséis por setor</legend>{setores.map((setor, setorIndice) => <article key={setor.slug}><header><strong>{setoresIndustria.find((item) => item.slug === setor.slug)?.titulo ?? setor.slug}</strong><span>{setor.midias.length} mídia(s)</span></header><label className="soltar-midia-cms adicionar-carrossel-cms" onDragOver={(evento) => evento.preventDefault()} onDrop={(evento) => { evento.preventDefault(); void enviarMidia(evento.dataTransfer.files[0], `setor:${setor.slug}`); }}><Plus size={20} /><strong>Adicionar imagem ou vídeo</strong><input type="file" accept="image/*,video/mp4,video/webm" disabled={enviandoMidia} onChange={(evento) => void enviarMidia(evento.target.files?.[0], `setor:${setor.slug}`)} /></label><div className="itens-midia-editaveis-cms">{setor.midias.map((midia, indice) => <div key={`${midia.src}-${indice}`}><span>{midia.tipo === 'video' ? 'Vídeo' : 'Imagem'} {indice + 1}</span><label>Legenda<input required maxLength={180} value={midia.legenda} onChange={(evento) => setSetores((atuais) => atuais.map((atual, posicao) => posicao === setorIndice ? { ...atual, midias: atual.midias.map((item, itemIndice) => itemIndice === indice ? { ...item, legenda: evento.target.value } : item) } : atual))} /></label><label>Descrição acessível<input required maxLength={240} value={midia.alt} onChange={(evento) => setSetores((atuais) => atuais.map((atual, posicao) => posicao === setorIndice ? { ...atual, midias: atual.midias.map((item, itemIndice) => itemIndice === indice ? { ...item, alt: evento.target.value } : item) } : atual))} /></label><button type="button" onClick={() => setSetores((atuais) => atuais.map((atual, posicao) => posicao === setorIndice ? { ...atual, midias: atual.midias.filter((_, itemIndice) => itemIndice !== indice) } : atual))}><Trash2 size={14} /> Remover</button></div>)}</div></article>)}</fieldset>}
          {ehAcontecimentos && <fieldset className="editor-acontecimentos-cms"><legend>Acontecimentos exibidos na página inicial</legend>{itens.map((item, indice) => <article key={`${indice}-${item.tipo}`}><div className="linha-editor-cms"><label>Momento<select value={item.tipo} onChange={(evento) => alterarItem(indice, 'tipo', evento.target.value)}><option value="recente">Aconteceu</option><option value="agora">Está acontecendo</option><option value="proximo">Vai acontecer</option></select></label><label>Data ou período<input required value={item.data} onChange={(evento) => alterarItem(indice, 'data', evento.target.value)} /></label></div><label>Título<input required minLength={3} value={item.titulo} onChange={(evento) => alterarItem(indice, 'titulo', evento.target.value)} /></label><label>Resumo<textarea required minLength={10} rows={3} value={item.resumo} onChange={(evento) => alterarItem(indice, 'resumo', evento.target.value)} /></label><label className="soltar-midia-cms adicionar-carrossel-cms"><Plus size={20} /><strong>Adicionar mídia a este acontecimento</strong><input type="file" accept="image/*,video/mp4,video/webm" disabled={enviandoMidia} onChange={(evento) => void enviarMidia(evento.target.files?.[0], `acontecimento:${indice}`)} /></label><div className="itens-midia-editaveis-cms">{(item.midias ?? []).map((midia, midiaIndice) => <div key={`${midia.src}-${midiaIndice}`}><span>{midia.tipo === 'video' ? 'Vídeo' : 'Imagem'} {midiaIndice + 1}</span><label>Legenda<input required maxLength={180} value={midia.legenda} onChange={(evento) => setItens((atuais) => atuais.map((atual, posicao) => posicao === indice ? { ...atual, midias: (atual.midias ?? []).map((registro, registroIndice) => registroIndice === midiaIndice ? { ...registro, legenda: evento.target.value } : registro) } : atual))} /></label><label>Descrição acessível<input required maxLength={240} value={midia.alt} onChange={(evento) => setItens((atuais) => atuais.map((atual, posicao) => posicao === indice ? { ...atual, midias: (atual.midias ?? []).map((registro, registroIndice) => registroIndice === midiaIndice ? { ...registro, alt: evento.target.value } : registro) } : atual))} /></label><button type="button" onClick={() => setItens((atuais) => atuais.map((atual, posicao) => posicao === indice ? { ...atual, midias: (atual.midias ?? []).filter((_, registroIndice) => registroIndice !== midiaIndice) } : atual))}><Trash2 size={14} /> Remover</button></div>)}</div>{itens.length > 1 && <button type="button" className="remover-item-cms" onClick={() => setItens((atuais) => atuais.filter((_, posicao) => posicao !== indice))}><Trash2 size={15} /> Remover acontecimento</button>}</article>)}<button type="button" className="adicionar-item-cms" onClick={() => setItens((atuais) => [...atuais, { tipo: 'proximo', data: '', titulo: '', resumo: '', midias: [] }])}><Plus size={15} /> Adicionar acontecimento</button></fieldset>}
