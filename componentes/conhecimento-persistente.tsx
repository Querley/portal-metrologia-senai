...
export function ConhecimentoPersistente({ cliente, perfil }: { cliente: SupabaseClient; perfil: PerfilInterno }) {
  ...
  const indicadoresVisiveis = useMemo(() => indicadores.filter((item) => correspondeBusca(busca, item.solicitacao_codigo, item.empresa_nome, item.servico_slug, item.licao_resumo, item.licao_assuntos)
    && (filtroLicao === 'todos' || (filtroLicao === 'sem_licao' ? !item.licao_estado : item.licao_estado === filtroLicao))
    && (filtroAssertividade === 'todos' || (filtroAssertividade === 'assertivo' ? item.esforco_assertivo === true : item.esforco_assertivo === false))), [busca, filtroAssertividade, filtroLicao, indicadores]);
  ...
  return <div className="painel conhecimento-persistente">
    ...
    <section className="bloco lista-indicadores"><header><div><h2>Estimado versus realizado</h2><p>Custos aparecem somente para Validador e Administrador.</p></div><button type="button" onClick={() => void carregar()}><RefreshCw size={15} /> Atualizar</button></header>
      <BarraBuscaFiltros busca={busca} aoMudarBusca={setBusca} placeholder="Pesquisar protocolo, empresa, serviço ou assunto" total={indicadoresVisiveis.length} filtros={[{ id: 'estado-licao', rotulo: 'Lição', valor: filtroLicao, aoMudar: setFiltroLicao, opcoes: [{ valor: 'todos', rotulo: 'Todas as lições' }, { valor: 'sem_licao', rotulo: 'Sem lição' }, { valor: 'em_validacao', rotulo: 'Em validação' }, { valor: 'formalizada', rotulo: 'Formalizadas' }] }, { id: 'assertividade', rotulo: 'Esforço', valor: filtroAssertividade, aoMudar: setFiltroAssertividade, opcoes: [{ valor: 'todos', rotulo: 'Toda assertividade' }, { valor: 'assertivo', rotulo: 'Dentro de ±15%' }, { valor: 'fora', rotulo: 'Fora de ±15%' }] }]} />
