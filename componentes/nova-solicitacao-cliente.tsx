...
export function NovaSolicitacaoCliente({ cliente, demonstracao = false, empresaNome, aoFechar, aoCriada }: Propriedades) {
  ...
  async function enviar(evento: React.FormEvent<HTMLFormElement>) {
    ...
    if (demonstracao) {
      const codigo = 285;
      await concluirComArquivos(
        {
          solicitacao_id: `demo-solicitacao-${Date.now()}`,
          codigo,
          protocolo: `DEM-SOL-${String(codigo).padStart(4, '0')}`,
        },
        dados,
        [],
      );
  ...
  return (
    <div
      className="fundo-nova-solicitacao"
      role="presentation"
      onMouseDown={(evento) => {
            ...
            <div className="campo-largo anexos-nova-solicitacao">
              <label className="seletor-anexos-cliente">
                ...
                <input
                  type="file"
                  multiple
                  accept=".pdf,.jpg,.jpeg,.png,.webp,.step,.stp,.iges,.igs,.stl,.obj,.dxf,.dwg"
                  onChange={(evento) => {
                    selecionarArquivos(evento.target.files);
                    evento.target.value = '';
                  }}
                />
              </label>
              {arquivos.length > 0 && (
                <ul>
                  {arquivos.map((arquivo, indice) => (
                    <li key={`${arquivo.name}-${arquivo.size}-${indice}`}>
                      <Paperclip size={15} />
                      <span>
