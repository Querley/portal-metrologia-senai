  ...
  return <main className="portal-cliente">
    ...
    <div className="conteudo-cliente">
      ...
      {selecionada && <div className="grade-cliente">
        <aside className="lista-projetos-cliente"><h2>Seus trabalhos</h2>{solicitacoes.map((item) => <button key={item.id} type="button" className={item.id === selecionada.id ? 'ativo' : ''} onClick={() => setSelecionadaId(item.id)}><span>SOL-{String(item.codigo).padStart(4, '0')}</span><strong>{item.servico}</strong><small>Recebida em {dataCurta(item.criada_em)}</small></button>)}</aside>
        <section className="detalhe-projeto-cliente">
          <header><div><span>SOL-{String(selecionada.codigo).padStart(4, '0')}</span><h2>{selecionada.servico}</h2></div><button type="button" onClick={() => void carregar()}><RefreshCw size={15} /> Atualizar</button></header>
          <div className="resumo-projeto-cliente"><article><small>Pré-proposta do laboratório</small><strong>{selecionada.valor_pre_proposta === null ? 'Em preparação' : formatarDinheiro(selecionada.valor_pre_proposta)}</strong><span>{selecionada.prazo_pagamento_dias ? `Pagamento desejado: ${selecionada.prazo_pagamento_dias} dias` : 'Condição em análise'}</span></article><article><small>Situação atual</small><strong>{selecionada.etapas.find((etapa) => etapa.estado === 'em_andamento')?.titulo ?? (selecionada.etapas.every((etapa) => etapa.estado === 'concluida') ? 'Concluído' : 'Aguardando início')}</strong><span>Atualização exibida pela equipe do laboratório</span></article></div>
          <section className="acompanhamento-etapas"><div className="titulo-bloco-cliente"><div><h3>Etapas do trabalho</h3><p>A porcentagem indica o progresso da etapa atual, não do contrato inteiro.</p></div></div><ol>{selecionada.etapas.map((etapa) => { const { rotulo, Icone } = estadoEtapa[etapa.estado]; return <li key={etapa.id} className={`etapa-${etapa.estado}`}><span className="icone-etapa"><Icone size={20} /></span><div><header><strong>{etapa.titulo}</strong><b>{rotulo}{etapa.estado === 'em_andamento' ? ` (${etapa.progresso}%)` : ''}</b></header>{etapa.descricao && <p>{etapa.descricao}</p>}<div className="barra-progresso" aria-label={`${etapa.progresso}% concluído`}><i style={{ width: `${etapa.progresso}%` }} /></div><small>Atualizado em {dataCurta(etapa.atualizada_em)}</small></div></li>; })}</ol></section>
