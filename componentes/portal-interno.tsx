...
type Estado = 'carregando' | 'sem_configuracao' | 'anonimo' | 'sem_perfil' | 'autenticado' | 'erro';
...
export function PortalInterno() {
  ...
  const carregarPerfil = useCallback(async (usuarioId: string) => {
    ...
    try {
      ...
      setEstado(perfilEncontrado ? 'autenticado' : 'sem_perfil');
    } catch {
...
function TelaAcesso({ estado, cliente, mensagem, aoAutenticar }: { estado: Estado; cliente: SupabaseClient | null; mensagem: string; aoAutenticar: (usuarioId: string) => Promise<void> }) {
  ...
  return <main className="acesso-interno"><section className="cartao-acesso"><a href="/" aria-label="Voltar à página pública"><MarcaOficial /></a><span className="selo-acesso"><ShieldCheck size={15} /> Área interna restrita</span>{estado === 'carregando' && <><h1>Validando acesso</h1><p role="status">Aguarde enquanto confirmamos sua sessão e seu perfil interno.</p></>}{estado === 'sem_configuracao' && <><h1>Integração de homologação pendente</h1><p>O acesso interno permanece fechado até a URL e a chave pública do Supabase de homologação serem configuradas.</p><a className="link-acesso" href="/portal/demonstracao">Abrir demonstração local</a></>}{estado === 'sem_perfil' && <><h1>Perfil interno não autorizado</h1><p>Sua identidade foi confirmada, mas não existe um perfil interno ativo e demonstrativo vinculado a ela.</p><button className="botao-acesso" type="button" onClick={() => void cliente?.auth.signOut()}>Sair</button></>}{estado === 'erro' && <><h1>Não foi possível validar o acesso</h1><p role="alert">{mensagem || 'A autenticação está temporariamente indisponível.'}</p><button className="botao-acesso" type="button" onClick={() => window.location.reload()}>Tentar novamente</button></>}{estado === 'anonimo' && <><h1>Entrar no Portal de Metrologia</h1><p>Use a conta interna provisionada no Supabase de homologação.</p><form onSubmit={entrar}><label htmlFor="email-interno">E-mail</label><input id="email-interno" type="email" autoComplete="username" required value={email} onChange={(evento) => setEmail(evento.target.value)} /><label htmlFor="senha-interna">Senha</label><input id="senha-interna" type="password" autoComplete="current-password" required value={senha} onChange={(evento) => setSenha(evento.target.value)} />{erro && <p className="erro-acesso" role="alert">{erro}</p>}<button className="botao-acesso" type="submit" disabled={enviando}><LockKeyhole size={16} />{enviando ? 'Validando…' : 'Entrar'}</button></form><a className="link-acesso" href="/portal/demonstracao">Prefiro abrir a demonstração local</a></>}</section></main>;
}
