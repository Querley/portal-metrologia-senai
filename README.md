...
O recorte de 28 de agosto–1º de setembro acrescenta descoberta por setores, solicitação sem login, dados mínimos da pré-proposta, VLibras e uma área do Cliente com perfil externo, etapas e mensagens bidirecionais persistentes. O documento emitido pelo laboratório é uma pré-proposta; a proposta oficial é criada no Nectar, sem integração com este portal. As migrations `202608280013` até `202609010023` estão aplicadas e registradas na homologação; a `021` acrescenta o aceite autenticado do Cliente e a confirmação do início somente pelo Administrador, a `022` cria modelos versionados de etapas por serviço e acompanhamento persistente em tempo real, e a `023` separa o fechamento registrado pela equipe da aprovação final por Validador ou Administrador. O fluxo público persiste somente dados sintéticos com e-mail `.test`, gera protocolo, vincula a solicitação ao mesmo e-mail autenticado, abre um canal protegido e permite que a equipe crie o orçamento no mesmo atendimento, sem duplicar a solicitação. O Administrador gera o PDF privado da versão aprovada, congela seu SHA-256 e o Cliente baixa o documento emitido com autorização por vínculo e verificação local de integridade.
